...
export const WebSocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    if(token){
      connectSocket("room", "ws://52.78.25.188:9098/api/ws/room");
      connectSocket("server", "ws://52.78.25.188:9098/api/ws/server");
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    if (userInfo?.userId) {
      connectSocket("match", `ws://52.78.25.188:9098/api/ws/match?userId=${userInfo.userId}`);
    }
  }, [token]);
  ...
  useEffect(() => {
    const userInfo = getUserInfoFromToken(token);
    if (userInfo?.userNo) {
      connectSocket("ban", `ws://52.78.25.188:9098/api/ws/ban?userNo=${userInfo.userNo}`);
    }
  }, [token]);
